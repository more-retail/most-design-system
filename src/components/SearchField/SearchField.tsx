...
function SearchField({
  ...
  // Clear button: always visible on md when populated; on sm only on hover / focus
  const showClear =
    hasValue && !disabled && (size === "md" || isFocused || isHovered);
  ...
  return (
    <div
      ...
      className={cn(
        "flex items-center rounded-full bg-neutral-10 transition-colors border-2",
        ss.height,
        ss.px,
        ss.gap,
        ss.width,
    ...
    >
      ...
      {showClear && (
        <button
          ...
          onClick={handleClear}
          className="shrink-0 flex items-center justify-center text-neutral-110"
        >
